...
enum AuditConfiguration {
    ...
    FILE_PATH("appender.file.path", String.class),
    FILE_MAX_SIZE("appender.file.maxsize", Long.class, 52428800L),
    FILE_MAX_BACKUP("appender.file.maxbackup", Integer.class, 20),
    LOG_LOCATION("location", Boolean.class, Boolean.FALSE),
    SOCKET_HOST("appender.socket.host", String.class, "localhost"),
    SOCKET_PORT("appender.socket.port", Integer.class, 4560),
    CONSOLE_PATTERN("appender.console.pattern", String.class, "%d{yyyy-MM-dd HH:mm:ss} %-5p %c - %m%n"),
    CONSOLE_TARGET("appender.console.target", LogTarget.class, LogTarget.OUTPUT),
    APPENDER("log.appender", LogAppenders.class) {
    ...
    public static void loadFromProperties(Properties props) {
        for (String key : props.stringPropertyNames()) {
            ...
            for (AuditConfiguration prop : AuditConfiguration.values()) {
                if (prop.getProperty().equalsIgnoreCase(key)) {
                    if (String.class.equals(prop.getClz())) {
                        ...
                        prop.setValue(Enum.valueOf((Class<? extends Enum>) prop.getClz(), value.toUpperCase()), prop.getClz());
                    } else {
