...
To do that, the implementation of the `AbstractBackend` have to override the method `isOnHoldForFormattedMessages` in order to return `false` (default value is `true` - backward compatibility considerations).
